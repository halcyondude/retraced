...
export default async function handlerRaw(req): Promise<any> {
  ...
  const token: ViewerToken = await createViewerDescriptor(auth, projectId, isAdmin, actorId, groupId, teamId, targetId, viewLogAction);
  return {
    status: 201,
