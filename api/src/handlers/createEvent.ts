...
export class EventCreater {
   ...
   */
  public async saveRawEvent(projectId: string, envId: string, eventInput: CreateEventRequest): Promise<void> {
  ...
  public async saveEvent(
    ...
    await instrument("EventCreater.insertOne", async () => {
      return await pg.query(insertStmt, insertVals);
    });
  ...
  public async appendEventToBacklog(
    ...
    await instrument("EventCreater.insertOneIntoBacklog", async () => {
      return await pg.query(insertStmt, [
        projectId,
        envId,
        newEventId,
        received,
