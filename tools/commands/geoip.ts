...
async function doQuery(argv) {
  ...
  pg.query(`select * from geoip where network >> '${argv.query}'`)
    .then((results) => {
      sp.stop();
      console.log();
...
function translateIPBlockData(filename, locations) {
  return new Promise((resolve, reject) => {
    ...
    let first = true;
    blockReader.on("line", async (line) => {
      // Skip CSV column names.
      if (first) {
        first = false;
        return;
